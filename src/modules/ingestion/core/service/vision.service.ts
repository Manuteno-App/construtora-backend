      ...
      messages: [
        {
          ...
          content:
            ...
            '===HEADER_JSON_START===\n{ "obra":"string|null", "objeto":"string|null", "contratante":"string|null", "contratada":"string|null", "cnpj":"string|null", "cnpj_contratada":"string|null", "contrato":"string|null", "numero_atestado":"string|null", "cidade":"string|null", "estado":"string|null", "local":"string|null", "valor_obra":"string|null", "valor_atestado":"string|null", "data_atestado":"DD/MM/YYYY|null", "data_inicio":"DD/MM/YYYY|null", "data_fim":"DD/MM/YYYY|null", "engenheiro":"string|null", "titulo":"string|null" }\n===HEADER_JSON_END===\n' +
            ...
            'For the header, capture objeto as the name or description of the project/work. Capture numero_atestado only for the certificate/CAT/attestation number, never the contract number. Capture valor_obra whenever the document states the total/global/contract/work value, preserving the amount exactly as printed (including R$ and Brazilian separators). Capture valor_atestado only when the document explicitly declares the value certified by the attestation; do not use table subtotals or individual service prices for either value. Include declared work length or KM start/end in the plain transcription so the entity extractor can calculate it. A document title such as Declaracao de Conclusao de Obras is never the object. Explicitly capture the document end or completion date as data_fim, including labels such as data de conclusao, termino, fim dos servicos, or prazo final. ' +
